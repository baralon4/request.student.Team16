...
describe('Services', () => {
  ...
  describe('logout', () => {
    ...
    test('removes user data from localStorage', () => {
      ...
      localStorage.setItem(services.USER_DATA_LOCAL_STORAGE_KEY, JSON.stringify({ key: 'test1' }))
      expect(services.getFromLocalStorage(services.USER_DATA_LOCAL_STORAGE_KEY)).toMatchObject({ key: 'test1' });
  ...
  describe('Validators', () => {
    ...
    test('exists', () => {
      ...
      expect(services.Validators.exists('')).toBe(false);
      expect(services.Validators.exists('test')).toBe(true);
    });
  })
})
