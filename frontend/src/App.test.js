...
describe('Services', () => {
  ...
  describe('logout', () => {
    ...
    test('removes user data from localStorage', () => {
      ...
      localStorage.setItem(services.USER_DATA_LOCAL_STORAGE_KEY, JSON.stringify({ key: 'example' }))
      expect(services.getFromLocalStorage(services.USER_DATA_LOCAL_STORAGE_KEY)).toMatchObject({ key: 'example' });

      services.logout()
      expect(services.getFromLocalStorage(services.USER_DATA_LOCAL_STORAGE_KEY)).toBe(null);
  ...
  describe('Validators', () => {
    ...
    test('exists', () => {
      ...
      expect(services.Validators.exists('')).toBe(false);
      expect(services.Validators.exists('example')).toBe(true);
    });
  })
})